...
The predict file is the entry point for predicting an image. It predicts an image 
passed by the user using a saved checkpoint or model.

The predict module requires two positional arguments and 3 optional araguments to 
be passed as input from the user running the program from a terminal window. The 
module predicts an image based on athe image and checkpoint provided, it predicts 
the image with the default setting or according to the optional arguments passed by 
the user such as the numbar of top K predictions to return, the image label mapping 
and whether to use GPU for the training. The predict module finally prints the top 
prediction, the prediction accuracy and a table listing the Top K classes and their 
respective probability.

FORMAT
------
./predict.py input checkpoint [--top_k TOP_K] [--category_names CATEGORY_NAMES] [--gpu]
...
- top_k(int) (optional): The number of top most likely predicted classes
    - default: 5
- category_names(str) (optional): Path/Location of the mapping of categories to names
    - default: 'cat_to_name'
...
def main():
    ...
    if class_cat is not None and class_cat in cat_to_name:
        ...
        print('[-] {:>25}\t{:<20}'.format('Image Class:', cat_to_name[class_cat]))
    ...
    if class_cat is not None and class_cat in cat_to_name:
        ...
        print('[-] {:>25}\t{:<10}\n'.format('Prediction Asessment:', 'Correct' if cat_to_name[class_cat] == names[0] else 'Incorrect'))
    else:
        print()
