...
The train file is the entry point for training a model. It trains a model using 
a selected model architecture, the available model architectures are VGG (vgg11), 
ALEXNET and DENSENET (densenet121).

The train module requires one positional argument and 6 optional araguments to 
be passed as input from the user running the program from a terminal window. The 
module trains the model based on athe training data provided and trains the model 
with the default setting or according to the optional arguments passed by the user 
such as the architecture, laerning rate, number of hidden units, total training epocch 
and whether to use GPU for the training. It displays the training loss, vvalidation loss, 
and validation accuracy while training. After training is complete it saves the trained 
model and test it and displays the test accuracy.

FORMAT
------
./train.py data_dir [--save_dir SAVE_DIR] [--arch {vgg,alexnet,densenet}]
[--learning_rate LEARNING_RATE] [--hidden_units HIDDEN_UNITS] [--epochs EPOCHS] [--gpu]
...
def main():
    ...
    try:
        ...
        # If training is interrupted by the user ask the user whether to save the 
        ...
        # If training completes without interuption or exception it is saved automatically
        saved, checkpoint_path = train_utils.save_model(model.cpu(), optimizer, image_datasets, input_arguments.arch, input_arguments.save_dir, input_size)
